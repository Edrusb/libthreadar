...
namespace libthreadar
{
    ...
    template <class T> void ratelier_gather<T>::worker_fill(unsigned int slot, std::unique_ptr<T> one)
    {
	verrou.lock();
    ...
    template <class T> void ratelier_gather<T>::get(std::vector<std::unique_ptr<T> > & ones)
    {
	ones.clear();
